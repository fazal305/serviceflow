...
export async function getReportsSummary(): Promise<ReportsSummary> {
  ...
  const [revenueByMonth, completedJobs, jobsByTechnician, jobsByCategory, pendingPayments, avgCompletion] =
    await Promise.all([
      ...
      pool.query<{ count: string }>(`SELECT count(*) FROM service_requests WHERE status = 'COMPLETED'`),
      pool.query<{ technician: string; count: string }>(
      ...
      pool.query<{ avg_days: string | null }>(
