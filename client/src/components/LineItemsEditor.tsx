...
import type { LineItemType } from '../api/quotations';
...
export function LineItemsEditor({
  ...
  const [localItems, setLocalItems] = useState<LineItemDraft[]>(items.length ? items : [emptyItem()]);
    ...
    <div className="flex flex-col gap-2">
      {localItems.map((item, i) => (
        <div key={i} className="grid grid-cols-12 gap-2">
          <select
            value={item.type}
            onChange={(e) => update(i, { type: e.target.value as LineItemType })}
