...
const KPI_CARDS: Array<{ key: 'openRequests' | 'pendingAssignment' | 'activeTechnicians' | 'totalCustomers'; label: string }> = [
          ...
          <ul className="mt-4 flex flex-col gap-2">
            {data.statusBreakdown.map((row) => (
              <li key={row.status} className="flex items-center justify-between">
                <StatusBadge status={row.status as ServiceRequestStatus} />
